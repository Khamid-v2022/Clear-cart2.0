					...
					<div style="width: 100%">
						<div class="card">
							...
							<div class="card-footer text-muted">
								{{ $ticket->getDateTime() }} | {{ $ticket->getUser()->name }} | <b>{{
									__('backend/management.tickets.edit.category') }}</b> {{
						...
						<form method="POST" class="kt-form" action="{{ route('backend-management-ticket-move-form') }}"
							style="width: 100%">
							@csrf
							...
							<div class="form-group" style="width: 100%">
								<label for="ticket_move_category">{{ __('backend/management.tickets.edit.move_category')
									}}</label>
								<select style="width: 100%"
									class="form-control @if($errors->has('ticket_move_category')) is-invalid @endif"
									id="ticket_move_category" name="ticket_move_category">
									<option value="0">{{ __('frontend/main.please_choose') }}</option>
									@foreach(\App\Models\UserTicketCategory::orderBy('name')->get() as
									$userTicketCategory)
									<option value="{{ $userTicketCategory->id }}" @if($ticket->category_id ==
										$userTicketCategory->id) selected @endif>{{ $userTicketCategory->name }}
									</option>
									@endforeach
								</select>
							...
							<button type="submit" class="btn btn-wide btn-bold btn-danger">{{
								__('backend/management.tickets.edit.move') }}</button>
						</form>
